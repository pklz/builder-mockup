...
/**
 * Module Proxy handles the drag/dropping from the module list to the layer (in the RuleBuilder)
 * @class ModuleProxy
 * @constructor
 * @param {HTMLElement} el
 * @param {WireIt.RuleBuilder} RuleBuilder
 */
WireIt.ModuleProxy = function(el, RuleBuilder) {
...
YAHOO.extend(WireIt.ModuleProxy, YAHOO.util.DDProxy, {
   ...
   /**
    * Add the module to the RuleBuilder on drop on layer
    * @method onDragDrop
    */
   onDragDrop: function(e, ddTargets) { 
      ...
      this._RuleBuilder.addModule( this._module ,[pos[0]-layerPos[0]+layer.el.scrollLeft, pos[1]-layerPos[1]+layer.el.scrollTop]);
    }
...
/**
 * The RuleBuilder class provides a full page interface 
 * @class RuleBuilder
 * @constructor
 * @param {Object} options
 */
WireIt.RuleBuilder = function(options) {
	...
	// TO DO: retrieve this from a web service
	this.rules = {
		libraryRules: [
			{ 
				name: "Category 1",
				...
				children: [
					{ name: "Rule 1", type: this.RULE_TYPE },
					{ name: "Rule 2", type: this.RULE_TYPE },
					{ name: "Rule 3", type: this.RULE_TYPE }
				]
			}, { 
				name: "Category 2",
				type: this.CATEGORY_TYPE,
				children: [
					{ name: "Rule 4", type: this.RULE_TYPE },
					{ name: "Rule 5", type: this.RULE_TYPE },
				]
			}
		],
		myRules: [
			{
				...
				type: this.CATEGORY_TYPE,
				children: [
					{ name: "My Rule 1", type: this.RULE_TYPE },
					{ name: "My Rule 2", type: this.RULE_TYPE }
				]
			}
		]
	};
...
WireIt.RuleBuilder.prototype = {
 ...
 setOptions: function(options) {
    ...
    for(var i = 0 ; i < this.modules.length ; i++) {
	 ...
	 this.adapter = options.adapter || WireIt.RuleBuilder.adapters.JsonRpc;
    ...
    this.options.layoutOptions = options.layoutOptions || {
	 	units: [
	      ...
	      { position: 'left', width: 320, resize: true, body: 'left', gutter: '5px', collapse: true, 
	        collapseSize: 25, header: 'Rules', scroll: true, animate: true },
	      { position: 'center', body: 'center', gutter: '5px' },
	      { position: 'right', width: 200, resize: true, body: 'right', gutter: '5px', collapse: true, 
 ...
 /**
  * Build the modules menu on the left
...
buildRulesTree: function() {
	var libraryRules = this.rules.libraryRules;
	this.libraryRulesTree = new YAHOO.widget.TreeView("libraryRules");
	for(var i = 0; i < libraryRules.length; i++) {
		this.populateChildren(libraryRules[i], this.libraryRulesTree.getRoot());
	}	

	this.libraryRulesTree.draw();

	var myRules = this.rules.myRules;
	this.myRulesTree = new YAHOO.widget.TreeView("myRules");
	for(var i = 0; i < myRules.length; i++) {
		this.populateChildren(myRules[i], this.myRulesTree.getRoot());
	}	

	this.myRulesTree.draw();
},

populateChildren: function(jsonNode, parent) {
	var node = new YAHOO.widget.MenuNode(jsonNode.name, parent, false);
	if(jsonNode.type == this.CATEGORY_TYPE && jsonNode.children) {
		for(var i = 0; i < jsonNode.children.length; i++) {
			this.populateChildren(jsonNode.children[i], node);
		}
	}
},

 /**
  * Add a module definition to the right list
  */
 addModuleToList: function(module) {
	
	var div = WireIt.cn('div', {className: "RuleBuilder-module"});
 ...
 addModule: function(module, pos) {
    try {
       ...
       Dom.addClass(container.el, "RuleBuilder-module-"+module.name);
    }
    catch(ex) {
 ...
 renderButtons: function() {
    ...
    // Buttons :
    var newButton = new widget.Button({ label:"New", id:"RuleBuilder-newButton", container: toolbar });
    newButton.on("click", this.onNew, this, true);

    var loadButton = new widget.Button({ label:"Load", id:"RuleBuilder-loadButton", container: toolbar });
    loadButton.on("click", this.load, this, true);

    var saveButton = new widget.Button({ label:"Save", id:"RuleBuilder-saveButton", container: toolbar });
    saveButton.on("click", this.onSave, this, true);
    var deleteButton = new widget.Button({ label:"Delete", id:"RuleBuilder-deleteButton", container: toolbar });
    deleteButton.on("click", this.onDelete, this, true);

    var helpButton = new widget.Button({ label:"Help", id:"RuleBuilder-helpButton", container: toolbar });
 ...
 renderLoadPanel: function() {
    if( !this.loadPanel) {
       this.loadPanel = new widget.Panel('RuleBuilder-loadPanel', {
          fixedcenter: true,
          draggable: true,
          width: '500px',
          visible: false,
 ...
 loadPipe: function(name) {
    ...
    if(lang.isArray(wiring.modules)) {
       ...
       for(i = 0 ; i < wiring.modules.length ; i++) {
          ...
          if(this.modulesByName[m.name]) {
             ...
             Dom.addClass(container.el, "RuleBuilder-module-"+m.name);
             container.setValue(m.value);
          }
          else {
             throw new Error("RuleBuilder: module '"+m.name+"' not found !");
          }
       }
     ...
     * @type {YAHOO.widget.Panel}
     */
		this.alertPanel = new widget.Panel('RuleBuilder-alertPanel', {
         fixedcenter: true,
         draggable: true,
         width: '500px',
         visible: false,
...
/**
 * RuleBuilder Adapters
 * @static
 */
WireIt.RuleBuilder.adapters = {};
